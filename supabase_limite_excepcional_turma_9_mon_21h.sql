-- Exceção temporária: permite 5 alunos na TURMA 9 - MON 21H.
-- Internamente, essa turma ativa corresponde ao class_number = 48.
-- Mantém também a exceção já existente da turma 73 (THU 21H).
...
begin
  if not exists (
    ...
    where tc.class_number = 48
      and tc.is_active = true
      and tc.class_type = 'quartet'
      and tc.class_weekday = 1
      and tc.class_start_time = time '21:00'
  ) then
    raise exception 'A turma 48 não corresponde à TURMA 9 - MON 21H.';
  end if;
end;
$$;
...
begin
  if new.class_number is null then return new; end if;
...
revoke execute on function public.get_group_classes_with_available_spots() from public, anon;
...
revoke execute on function public.get_available_group_classes_for_students() from public, anon;
...
revoke execute on function public.switch_my_group_class(integer) from public, anon;
grant execute on function public.switch_my_group_class(integer) to authenticated;
