...
-- Cria conjuntos pessoais para alunos e conjuntos compartilhados pelo professor.
...
create index if not exists flashcard_decks_shared_updated_idx
  on public.flashcard_decks(is_shared, updated_at desc)
...
revoke all on function public.save_flashcard_deck(uuid, text, text, boolean, jsonb) from public, anon;
grant execute on function public.save_flashcard_deck(uuid, text, text, boolean, jsonb) to authenticated;
